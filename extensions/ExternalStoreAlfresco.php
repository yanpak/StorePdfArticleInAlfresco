...
class ExternalStoreAlfresco {
	...
	public function & store($store, $data) {
		...
		$res = $this -> session -> save();

		if ($res == FALSE || $res == null) {
			// Error was happened during saving node. At least it means that node doesn't exist in the repository. We should to create child and repeat all previuos steps
			$this->__construct();
			$node = $this -> wikiSpace -> createChild("cm_content", "cm_contains", "cm_" . $_SESSION["title"]);
			$node -> cm_name = $_SESSION["title"];
		...
		// Единственное дополнение. Необходимо для моего расширения StoreArticleInPdf. Ян Пак
		$_SESSION["lastNode"] = $node -> __toString();
		return $result;
	}
